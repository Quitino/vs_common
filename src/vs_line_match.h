#ifndef __VS_LINE_MATCH__
#define __VS_LINE_MATCH__
...
struct LineSegment2D
{
    LineSegment2D(double x1, double y1, double x2, double y2)
        : p1(x1, y1), p2(x2, y2) {cal();}
    LineSegment2D(const cv::Point2f& _p1, const cv::Point2f& _p2)
        : p1(_p1), p2(_p2) {cal();}
...
typedef std::vector<LineSegment2D> LineSegment2DList;

bool near(const LineSegment2D& l1, const LineSegment2D& l2,
          double max_angle = 0.05, double max_dist = 0.1, double min_overlap = 0);

double distance(const LineSegment2D& l1, const LineSegment2D& l2,
                double max_angle = 0.05, double max_dist = 0.1, double min_overlap = 0,
                double w_angle = 1, double w_dist = 1);
...
class LineSegment2DNN
{
public:
    virtual ~LineSegment2DNN(){}

    virtual void build(const LineSegment2DList& linelist) = 0;

    virtual bool nearest(const LineSegment2D& lquery, LineSegment2D& lnearest,
                 double max_angle, double max_dist) = 0;
};
...
public:
    ...
    ICLSolver(const LineSegment2DList& model, bool store_grid = false)
    : m_proj_affine(true)
    {
        setModel(model, store_grid);
    }

    void setModel(const LineSegment2DList& model, bool store_grid = false);

    bool match(const LineSegment2DList& target, cv::Point3f& transform,
               bool use_init_guess = false, double thres_angle = 0.3,
               double thres_dist = 1);
...
private:
    std::shared_ptr<LineSegment2DNN> m_map;
    bool m_proj_affine;
    cv::Mat m_fisher_info;
};
} /* namespace vs */

#endif//__VS_LINE_MATCH__