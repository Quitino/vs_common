               ...
               int lane_type, float min_lane_w, float max_lane_w, bool draw)
{
    ...
    auto foo_color = [](int type)
    {
        switch(type)
        {
            case 0: return defaultColorModel(100);
            case 1: return defaultColorModel(101);
            case 2: return defaultColorModel(102);
            case 3: return defaultColorModel(103);
            case 10: return defaultColorModel(110);
            default: return defaultColorModel(103);
        }
    };
    ...
    if(draw)
    {
        ...
        for(auto l : lines)
        {
            cv::line(img_show, cv::Point2f(l[0], l[1]), cv::Point2f(l[2], l[3]),
                    cv::Scalar(0, 255, 0), 2);
        }
        for(auto lane : lanes)
        {
        ...
        {
             cv::line(img_lsd, cv::Point2f(l[0], l[1]), cv::Point2f(l[2], l[3]),
                    cv::Scalar(0, 255, 0), 2);
        }
        cv::imshow("lane_detect", img_show);
        cv::imshow("mask", color_mask);
        cv::imshow("lsd", img_lsd);
        printf("detect lines:%d lanes:%d final:%d\n", (int)lines.size(), nlanes_raw, nlanes);
    }
    return nlanes;
}

static inline bool valid(float x1, float y1, float x2, float y2, const cv::Mat& mask,
                        float min_len = 0, float min_rate = MIN_MASK_RATE)
{
    ...
    {
        if(p.x < 0 || p.y < 0 || p.x >= mask.cols || p.y >= mask.rows) cnt_skip++;
        else if(mask.at<uchar>(p)) cnt++;
    }
    int sum = k + 1 - cnt_skip;
    if(sum < 1) return false;
    float rate = (float)cnt / sum;
    return rate > min_rate;
}

static bool valid(const cv::Point2f& p1, const cv::Point2f& p2, const cv::Mat& mask,
                  float min_len = 0, float min_rate = MIN_MASK_RATE)
{
    return valid(p1.x, p1.y, p2.x, p2.y, mask, min_len, min_rate);
}

static bool valid(const cv::Vec4f& l, const cv::Mat& mask, float min_len = 0,
                       ...
                       float max_lane_x)
{
    ...
    struct LinePack
    {
        ...
        void print()
        {
            printf("u1(%.1f %.1f) u2(%.1f %.1f) p1(%.3f %.3f) p2(%.3f %.3f) "
                   "dir:(%.3f %.3f) %.1f len:%.2f\n",
                   u1.x, u1.y, u2.x, u2.y, p1.x, p1.y, p2.x, p2.y,
                   dir.x, dir.y, rad2deg(theta), len);
        }
...
static void mergeLane(LaneList& lanes, const cv::Mat& mask)
{
    auto foo_need_merge = [mask](const Lane& a, const Lane& b)
    {
        ...
        float d[4] = {dist(a.p1, b.p1), dist(a.p2, b.p2),
                      dist(a.p1, b.p2), dist(a.p2, b.p1)};
        float dmin = std::min(std::min(d[0], d[1]), std::min(d[2], d[3])) + 1e-3;
    ...
    auto foo_merge = [](const Lane& a, const Lane& b)
    {
        ...
        if(kmax < k2) kmax = k2;
        cv::Point2f p1, p2;
        if(fequal(kmin, 0)) p1 = a.p1;
        else if(fequal(kmin, k1)) p1 = b.p1;
        else p1 = b.p2;
        if(fequal(kmax, 1)) p2 = a.p2;
        else if(fequal(kmax, k1)) p2 = b.p1;
        else p2 = b.p2;
