...
{
    double dp = p1 - p2;
    while(dp <= -M_PI_2) dp += M_PI;
    while(dp > M_PI_2) dp -= M_PI;
    return dp;
}

bool near(const LineSegment2D& l1, const LineSegment2D& l2,
...
double distance(const LineSegment2D& l1, const LineSegment2D& l2,
                double max_angle, double max_dist, double min_overlap,
...
bool solveTransform(const LineSegment2DList& model, const LineSegment2DList& target,
                    cv::Point3f& T)
{
    int N = model.size();
                    ...
                    cv::Point3f& T, cv::Mat& info)
{
    ...
    double temp[4] = {1.0 / std::max(eigvals.at<double>(0, 0), eps), 0, 0,
                      1.0 / std::max(eigvals.at<double>(1, 0), eps)};
...
static void transformLines(LineSegment2DList& lines, const Eigen::Matrix3d& T)
{
    for(auto& l : lines)
    {
...
static void projAffine(cv::Point3f& pose, cv::Point3f& raw_pose,
                       const LineSegment2DList& lines)
{
    for (size_t i = 0; i < lines.size() - 1; i++)
    {
        if (fabs(diff(lines[i].theta, lines[i + 1].theta)) > 0.05)
            return;
    }
    // TODO: Will it be more accurate to use average dir?
    cv::Point2f dir = lines[0].dir;
    cv::Point2f delta = cv::Point2f(raw_pose.x - pose.x, raw_pose.y - pose.y).dot(dir)
                        * dir;
    pose.x += delta.x;
    pose.y += delta.y;
}
...
bool ICL(const LineSegment2DList& model, const LineSegment2DList& target,
         cv::Point3f& transform, bool use_init_guess,
...
{
public:
    virtual void build(const LineSegment2DList& linelist)
    {
        m_list = linelist;
    }

    virtual bool nearest(const LineSegment2D& lquery, LineSegment2D& lnearest,
                 double max_angle, double max_dist);

private:
    LineSegment2DList m_list;
};


class LineNNGrid: public LineSegment2DNN
{
public:
    virtual void build(const LineSegment2DList& linelist);
...
private:
    std::vector<LineSegment2DList> m_grid;
...
class LineNNThetaD: public LineSegment2DNN
{
...
private:
    std::vector<std::vector<LineSegment2DList>> m_grid;
    double m_angle_min, m_angle_max, m_angle_step;
...
void ICLSolver::setModel(const LineSegment2DList& model, bool store_grid)
{
    if(model.empty()) return;
    if(store_grid) m_map.reset(new LineNNGrid);
    else m_map.reset(new LineSegStoreList);
    m_map->build(model);
}

bool ICLSolver::match(const LineSegment2DList& target, cv::Point3f& transform,
                      bool use_init_guess, double thres_angle, double thres_dist)
{
    cv::Point3f raw_transform = transform;
    LineSegment2DList lines = target;
    Eigen::Matrix3d T = Eigen::Matrix3d::Identity();
...
bool LineSegStoreList::nearest(const LineSegment2D& lquery,
            LineSegment2D& lnearest, double max_angle, double max_dist)
{
    double nearest_dist = 1e10;
    const LineSegment2D* nearest_ptr = NULL;
...
void LineNNGrid::build(const LineSegment2DList& model)
{
    if(model.empty()) return;
    m_angle_min = - M_PI_2;
    m_angle_max = M_PI_2;
...
bool LineNNGrid::nearest(const LineSegment2D& lquery,
                LineSegment2D& lnearest, double max_angle, double max_dist)
{
    double nearest_dist = 1e10;
    const LineSegment2D* nearest_ptr = NULL;
...
void LineNNThetaD::build(const LineSegment2DList& model)
{
    if(model.empty()) return;
...
bool LineNNThetaD::nearest(const LineSegment2D& lquery,
                LineSegment2D& lnearest, double max_angle, double max_dist)
{
    double nearest_dist = 1e10;
    const LineSegment2D* nearest_ptr = NULL;
    ...
    if(!index(theta_min, d_min, imin, jmin)
       || !index(theta_max, d_max, imax, jmax)) return false;
