...
StereoRectifier::StereoRectifier(const char* calib_file)
:m_rectified(false)
{
    init(calib_file);
}

StereoRectifier::StereoRectifier(const StereoVioCalib& calib_raw)
:m_rectified(false)
{
    init(calib_raw);
}
...
void StereoRectifier::calcRectify()
{
    ...
    else if(c.distort_model == "equidistant")
    {
        cv::fisheye::stereoRectify(c.K0, c.D0, c.K1, c.D1, c.img_size, c.R_c0_c1, c.t_c0_c1,
                                   R0, R1, P0, P1, Q, cv::CALIB_ZERO_DISPARITY, c.img_size, 0, 1);
...
bool loadStereoVioCalib(const char* calib_file, StereoVioCalib& calib)
{
    ...
    cv::Mat R,t;
    T2Rt(vec2T(vec_T_imu_cam0), R, t);
    calib.R_c0_imu = R.t();
    calib.t_c0_imu = -R.t()*t;
    T2Rt(vec2T(vec_T_cam0_cam1), calib.R_c0_c1, calib.t_c0_c1);
    T2Rt(vec2T(vec_T_imu_body), calib.R_imu_body, calib.t_imu_body);
    return true;
}
...
bool saveStereoVioCalib(const char* calib_file, const StereoVioCalib& calib)
{
    ...
    fprintf(fp, "  timeshift_cam_imu: 0.0\n");
