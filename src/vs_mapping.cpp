...
GridMapping<ValType, D>
::GridMapping(int len_exp, float res,
              ValType val_max, ValType val_inc,
              ValType val_dec, ValType val_thres)
: E(len_exp)
, K(1 << E)
, M(K - 1)
, UPPER(K >> 1)
...
{
    // calc dim size
    m_dim_size[D - 1] = sizeof(ValType);
    for(int i = D - 2; i >= 0; i--)
    {
        m_dim_size[i] = m_dim_size[i + 1] * K;
    }
    // clear map
    clear();
...
int GridMapping<ValType, D>::checkPoint(const float p[D], float r)
{
    ...
    for(int d = 0, tres; d < D; d++)
    {
        ...
        tp[d] = p[d] - r;
        tres = checkPoint(tp);
        if(tres == OCCUPY) return tres;
        else if(tres == FREE) res = tres;
...
int GridMapping<ValType, D>::checkLine(const float p1[D], const float p2[D], float r)
{
    ...
    else if(D == 3)
    {
        ...
        if(dx != 0 || dy != 0)
        {
            ...
            step_b[1] = dy * dz;
            step_b[2] = ss;
            k = m_res / std::sqrt(step_b[0] * step_b[0]
                                + step_b[1] * step_b[1]
                                + step_b[2] * step_b[2]);
            step_b[0] *= k;
            step_b[1] *= k;
...
template<class ValType, int D>
int GridMapping<ValType, D>::checkLineTravel(const float p1[D], const float p2[D],
...
void GridMapping<ValType, D>::moveMap(const float new_origin[3])
{
    ...
    for(int i = 0; i < D; i++)
        m_origin_base[i] = int(std::floor(m_origin[i] / m_res)) * m_res;
}
...
void GridMapping<ValType, D>::clearRow(int r, int dim)
{
    ...
    case 0:
        {
            int idx[D] = {r};
            memset(m_map + index(idx), 0, m_dim_size[dim]);
        }
        break;
    case 1:
        for(int i = 0; i < K; i++)
