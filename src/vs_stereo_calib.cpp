...
StereoRectifier::StereoRectifier(bool enable_cl)
: m_rectified(false), m_enable_cl(enable_cl)
{}

StereoRectifier::StereoRectifier(const char* calib_file, bool enable_cl)
: m_rectified(false), m_enable_cl(enable_cl)
{
    init(calib_file);
}
...
{
    if(loadStereoVioCalib(calib_file, m_calib_raw))
        calcRectify();
    return m_rectified;
}
...
void StereoRectifier::rectify(const cv::Mat& img0, const cv::Mat& img1,
                                    cv::Mat& out0, cv::Mat& out1)
{
    ...
    if(s0 != sc || s1 != sc)
    {
        printf("[ERROR]StereoRectifier: Failed rectify, input size [%dx%d] [%dx%d]"
...
void StereoRectifier::calcRectify()
{
    ...
    {
        cv::stereoRectify(c.K0, c.D0, c.K1, c.D1, c.img_size, c.R_c0_c1, c.t_c0_c1,
                          R0, R1, P0, P1, Q, cv::CALIB_ZERO_DISPARITY, 0);
        cv::initUndistortRectifyMap(c.K0, c.D0, R0, P0, c.img_size, CV_16SC2,
                                    m_rmap[0][0], m_rmap[0][1]);
        cv::initUndistortRectifyMap(c.K1, c.D1, R1, P1, c.img_size, CV_16SC2,
                                    m_rmap[1][0], m_rmap[1][1]);
    }
    else if(c.distort_model == "equidistant")
    {
        cv::fisheye::stereoRectify(c.K0, c.D0, c.K1, c.D1, c.img_size, c.R_c0_c1, c.t_c0_c1,
                                   R0, R1, P0, P1, Q, cv::CALIB_ZERO_DISPARITY, c.img_size,
                                   0, 1);
        cv::fisheye::initUndistortRectifyMap(c.K0, c.D0, R0, P0, c.img_size,
                                             CV_16SC2, m_rmap[0][0], m_rmap[0][1]);
        cv::fisheye::initUndistortRectifyMap(c.K1, c.D1, R1, P1, c.img_size,
                                             CV_16SC2, m_rmap[1][0], m_rmap[1][1]);
    }
    else
    {
...
bool loadStereoVioCalib(const char* calib_file, StereoVioCalib& calib)
{
    ...
    calib.K0 = (cv::Mat_<double>(3, 3)<<cam0_intrinsics[0], 0, cam0_intrinsics[2],
                                    0, cam0_intrinsics[1], cam0_intrinsics[3],
                                    0, 0, 1);
    calib.D0 = cv::Mat(cam0_distortion_coeffs);
    calib.K1 = (cv::Mat_<double>(3, 3)<<cam1_intrinsics[0], 0, cam1_intrinsics[2],
